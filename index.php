<?php
   $Title = "";
   $MetaDescription = "";
   $MetaKeywords = "";
?>

<?php
   include __DIR__ . '/A_Layout/Header/header.php';
?>
	<!-- Main Header-->
	<header class="main-header header-style-one header-3">

		<!-- Header Lower -->
		<div class="header-lower">
			<!-- Main box -->
			<div class="main-box">
				
				<!--Nav Box-->
				<div class="nav-outer">
					<div class="logo-box">
						<div class="logo"><a #><img src="images/mainloho-off.png" alt="" title="Varun Global "></a></div>
					</div>

					<div class="outer-box">
						<nav class="nav main-menu">
							<ul class="navigation">
								<li><a #>Home</a></li>
								<li><a href="about">About Us</a></li>
								<li><a href="study-abroad-services">Study Abroad Services</a></li>
								<li><a href="financial-services">Finacial Services</a></li>
								<li><a href="countries">Countries</a></li>
							</ul>
						</nav>

						<a href="contact" class="theme-btn btn-style-one">Contact Us</a>

						<!-- Mobile Nav toggler -->
						<div class="mobile-nav-toggler"><span class="icon lnr-icon-bars"></span></div>
					</div>
				</div>
			</div>
		</div>
		<!-- End Header Lower -->

		<!-- Mobile Menu  -->
		<div class="mobile-menu">
			<div class="menu-backdrop"></div>

			<!--Here Menu Will Come Automatically Via Javascript / Same Menu as in Header-->
			<nav class="menu-box">
				<div class="upper-box">
					<div class="nav-logo"><a #><img src="images/footerlogo.png" alt=""></a></div>
					<div class="close-btn"><i class="icon fa fa-times"></i></div>
				</div>

				<ul class="navigation clearfix">
					<!--Keep This Empty / Menu will come through Javascript-->
				</ul>
				<ul class="contact-list-one">
					<li>
						<!-- Contact Info Box -->
						<div class="contact-info-box">
							<i class="icon lnr-icon-phone-handset"></i>
							<span class="title">Call Now</span>
							<a href="tel:+92880098670">+92 (8800) - 98670</a>
						</div>
					</li>
					<li>
						<!-- Contact Info Box -->
						<div class="contact-info-box">
							<span class="icon lnr-icon-envelope1"></span>
							<span class="title">Send Email</span>
							<a href="/cdn-cgi/l/email-protection#b4dcd1d8c4f4d7dbd9c4d5dacd9ad7dbd9"><span class="__cf_email__" data-cfemail="18707d7468587b777568797661367b7775">[email&#160;protected]</span></a>
						</div>
					</li>
					<li>
						<!-- Contact Info Box -->
						<div class="contact-info-box">
							<span class="icon lnr-icon-clock"></span>
							<span class="title">Send Email</span>
							Mon - Sat 8:00 - 6:30, Sunday - CLOSED
						</div>
					</li>
				</ul>


				<ul class="social-links">
					
					<li><li><a href="mailto:varunglobaledu@gmail.com">
              <i class="fa-solid fa-envelope"></i>
            </a></li></li>
					<li><a href="https://wa.me/918780907831" target="_blank" rel="noopener noreferrer">
  <i class="fa-brands fa-whatsapp"></i>
</a></li>
					<li><a href="https://www.instagram.com/varunglobalservices_nikol?igsh=MWZ4Zmg5a3Rnem9waA=="><i class="fab fa-instagram"></i></a></li>
				</ul>
			</nav>
		</div><!-- End Mobile Menu -->

		<!-- Sticky Header  -->
		<div class="sticky-header">
			<div class="auto-container">
				<div class="inner-container">
					<!--Logo-->
					<div class="logo">
						<a #><img src="images/mainlogo.png" alt="img"></a>
					</div>

					<!--Right Col-->
					<div class="nav-outer">
						<!-- Main Menu -->
						<nav class="main-menu">
							<div class="navbar-collapse show collapse clearfix">
								<ul class="navigation clearfix">
									<!--Keep This Empty / Menu will come through Javascript-->
								</ul>
							</div>
						</nav><!-- Main Menu End-->

						<!--Mobile Navigation Toggler-->
						<div class="mobile-nav-toggler"><span class="icon lnr-icon-bars"></span></div>
					</div>
				</div>
			</div>
		</div><!-- End Sticky Menu -->
	</header>
	<!--End Main Header -->

	<!--Hero Section -->
	<section class="hero-section bg-cover hero-3" style="background-image: url('images/home-3/hero/hero-bg.png');">
		<div class="row g-4 align-items-center">
			<div class="col-xl-7 col-lg-6">
				<div class="hero-content">
					<h1 class="wow fadeInUp" data-wow-delay=".2s">Your Global <span>Education</span> <br> Partner</h1>
					<div class="text-item wow fadeInUp" data-wow-delay=".4s">
						<p>
							Varun Global Education Services helps students choose the right academic path with honest guidance. We offer study abroad support, IELTS coaching, career counselling, and admission assistance.
						</p>
						<img src="images/home-3/hero/small-image.jpg" alt="img">
					</div>
					<div class="hero-button wow fadeInUp animated" data-wow-delay=".6s">
						<a href="contact" class="theme-btn btn-style-one text-white">
							Start Your Journey
							<span class="icon"><i class="flaticon-finance-arrow-right-svgrepo-com1-2"></i></span>
						</a>
					</div>
				</div>
			</div>
			<div class="col-xl-5 col-lg-6 wow fadeInUp" data-wow-delay=".3s">
				<div class="hero-image ed_top_img">
					<img src="images/home-3/hero/hero-image.png" alt="img">
					<div class="ellipse-shape">
						<img src="images/home-3/hero/ellipse-shape.png" alt="img">
					</div>
					<div class="text-circle">
						<img src="images/home-3/hero/text-circle.png" alt="img" class="animation__rotate">
						<img src="images/home-3/hero/star.png" alt="img" class="star">
					</div>
				</div>
			</div>
		</div>
		<div class="paper-shape">
			<img src="images/home-3/hero/paper-shape.png" alt="img">
		</div>
	</section>

	<!--Feature Section -->
	<section class="feature-section-3 fix">
		<div class="row g-4">
			<div class="col-lg-5">
				<div class="feature-box-3">
					<div class="arrow-shape">
						<img src="images/home-3/arrow-shape.png" alt="img">
					</div>
					<div class="client-image wow fadeInUp" data-wow-delay=".2s">
						<img src="images/home-1/client-1.png" alt="img">
						<img src="images/home-1/client-2.png" alt="img" class="icon-2">
						<img src="images/home-1/client-3.png" alt="img" class="icon-3">
						<img src="images/home-1/client-4.png" alt="img" class="icon-4">
					</div>
					<span class="wow fadeInUp" data-wow-delay=".4s"><b>120K</b> Active Clients</span>
					<a href="contact" class="theme-btn btn-style-one wow fadeInUp" data-wow-delay=".6s">
						Lets Talk With Us
						<span class="icon"><i class="flaticon-finance-arrow-right-svgrepo-com1-2"></i></span>
					</a>
				</div>
			</div>
			<div class="col-lg-7">
				<div class="row g-4">
					<div class="col-xl-6 col-lg-12 col-sm-6 wow fadeInUp" data-wow-delay=".3s">
						<div class="feature-service-card-items">
						<div class="icon">
							<i class="flaticon-business-028-briefcase"></i>
						</div>
						<div class="content">
							<h3><a href="study-abroad-services">Business Consulting</a></h3>
							<p>
								There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected 
							</p>
							<a href="study-abroad-services" class="text-btn">
								Read More
							</a>
						</div>
						</div>
					</div>
					<div class="col-xl-6 col-lg-12 col-sm-6 wow fadeInUp" data-wow-delay=".5s">
						<div class="feature-service-card-items">
							<div class="icon">
								<i class="flaticon-finance-business-expense-svgrepo-com-1"></i>
							</div>
							<div class="content">
								<h3><a href="study-abroad-services">business Strategy</a></h3>
								<p>
									There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected 
								</p>
								<a href="study-abroad-services" class="text-btn">
									Read More
								</a>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
	
	<!--About Section -->
	<section class="about-section fix section-padding">
		<div class="auto-container">
			<div class="row g-4">
				<div class="col-lg-6">
					<div class="image-column-3">
							<div class="inner-column">
							<div class="image1 overlay-anim wow fadeInUp" data-wow-delay=".3s">
								<img src="images/home-3/about/about-01.jpg" alt="Image">
							</div>
							<div class="info float-bob-x">
								<img src="images/home-3/about/about-info.png" alt="Image">
							</div>
							<div class="image2 wow fadeInUp" data-wow-delay=".5s">
								<img src="images/home-3/about/about-02.jpg" alt="Image">
							</div>
						</div>
					</div>
				</div>
				<div class="col-lg-6">
					<div class="about-content-3">
						<div class="sec-title mb-0">
							<h6 class="sub-title wow fadeInUp">
								<span class="triangle triangle1"></span>
								<span class="triangle triangle2"></span>
								Who We Are
							</h6>
							<h2 class="wow fadeInUp" data-wow-delay=".2s">Your Trusted Partner in Global Education</h2>
						</div>
						<p class="about-text wow fadeInUp" data-wow-delay=".4s">At Varun Global Education Services, we are committed to helping students achieve their academic and career goals with the right guidance. With over a decade of expertise, we provide personalized support for studying abroad, including admission assistance, IELTS coaching, education loans, and visa processing.
							Our transparent and student-first approach ensures that every step of your journey is smooth, informed, and successful.</p>
						<div class="about-block wow fadeInUp" data-wow-delay=".6s">
							<div class="inner-box">
								<div class="icon">
									<i class="flaticon-finance-creativity-1-1"></i>
								</div>
								<div class="content">
								<h4 class="title">10+ Years of Industry Experience</h4>
								<p class="text">We’ve guided thousands of students with proven results and expert insights.</p>
								</div>
							</div>
						</div>
						<div class="about-block wow fadeInUp" data-wow-delay=".8s">
							<div class="inner-box">
											<div class="icon">
												<i class="flaticon-finance-edit-tools-1"></i>
											</div>
								<div class="content">
								<h4 class="title">Complete End-to-End Support</h4>
								<p class="text">From choosing the right course to securing your visa, we’re with you at every step.</p>
								</div>
							</div>
						</div>
						<div class="skill-feature wow fadeInUp" data-wow-delay="1s">
							<h3 class="box-title">Business Success</h3>
							<div class="progress">
								<div class="progress-bar" style="width: 88%; animation: 2.6s ease 0s 1 normal none running animate-positive; opacity: 1;">
									<div class="progress-value"><span class="counter-number2">88</span>%</div>
								</div>
							</div>
						</div>
						<div class="info mt-50 wow fadeInUp" data-wow-delay=".2s">
							<a href="contact" class="theme-btn btn-style-one wow fadeInUp" data-wow-delay=".5s">
								Lets Talk With Us
								<span class="icon"><i class="flaticon-finance-arrow-right-svgrepo-com1-2"></i></span>
							</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	      <!-- Counter Section Start -->
      <section class="counter-section section-padding pt-0">
        <div class="auto-container">
          <div class="counter-wrapper">
            <div class="counter-box-1 wow bounceInUp" data-wow-delay="00ms" data-wow-duration="1500ms">
              <div class="count-box">
                <h2><span class="count-text" data-speed="3000" data-stop="1">0</span>M</h2>
                <p>Worldwide business Grow</p>
              </div>
            </div>
            <div class="counter-box-1 wow bounceInUp" data-wow-delay="200ms" data-wow-duration="1500ms">
              <div class="count-box">
                <h2><span class="count-text" data-speed="3000" data-stop="12">0</span>K+</h2>
                <p>Satisfied Clients</p>
              </div>
            </div>
            <div class="counter-box-1 wow bounceInUp" data-wow-delay="300ms" data-wow-duration="1500ms">
              <div class="count-box">
                <h2><span class="count-text" data-speed="3000" data-stop="119">0</span>M</h2>
                <p>Countries Served</p>
              </div>
            </div>
            <div class="counter-box-1 wow bounceInUp" data-wow-delay="400ms" data-wow-duration="1500ms">
              <div class="count-box">
                <h2>
                  <span class="count-text" data-speed="3000" data-stop="33">0</span>
                </h2>
                <p>Award winning contest</p>
              </div>
            </div>
          </div>
        </div>
      </section>
	  	
	<!-- Why Choose Us Section Start -->
	<section class="why-choose-section-2 section-padding fix bg-cover" style="background-image: url('images/home-3/service-bg-2.jpg');">
		<div class="circle-shape">
			<img src="images/home-2/choose-us/circle-shape.png" alt="img">
		</div>
		<div class="auto-container">
			<div class="why-choose-us-wrapper-2 p-0">
				<div class="row g-4 justify-content-between">
					<div class="col-xl-5">
						<div class="service-imagestyle-3 fix wow fadeInUp" data-wow-delay=".3s">
							<img src="images/home-3/service-image.jpg" alt="img">
						</div>
					</div>
					<div class="col-xl-7">
						<div class="choose-us-content-2">
							<div class="sec-title mb-0">
								<h6 class="sub-title text-white">
									<span class="triangle triangle1"></span>
									<span class="triangle triangle2"></span>
									Latest Services
								</h6>
								<h2 class="text-white wow fadeInUp" data-wow-delay=".2s">You Need Someone At with Strategic Support</h2>
							</div>
							<div class="row">
								<div class="col-xl-6 col-lg-6 col-md-6 wow fadeInUp">
									<div class="choose-us-box-style-2">
										<div class="icon">
											<i class="flaticon-finance-business-expense-svgrepo-com-1"></i>
										</div>
										<div class="content">
											<h3><a href="page-service-details.html"> IELTS Coaching</a></h3>
											<p>
												Expert coaching for all four modules with mock tests and flexible class options.
											</p>
										</div>
									</div>
								</div>
								<div class="col-xl-6 col-lg-6 col-md-6 wow fadeInUp" data-wow-delay=".2s">
									<div class="choose-us-box-style-2">
										<div class="icon">
											<i class=" lnr-icon-layers"></i>
										</div>
										<div class="content">
											<h3><a href="page-service-details.html">Admission Process</a></h3>
											<p>
												Personalized counselling to help you choose the right course and university.
											</p>
										</div>
									</div>
								</div>
								<div class="col-xl-6 col-lg-6 col-md-6 wow fadeInUp" data-wow-delay=".4s">
									<div class="choose-us-box-style-2">
										<div class="icon">
											<i class="flaticon-business-023-lifesaver"></i>
										</div>
										<div class="content">
											<h3><a href="page-service-details.html">Education Loan Assistance</a></h3>
											<p>
												Easy loan support through top banks & NBFCs—secured or unsecured options available.
											</p>
										</div>
									</div>
								</div>
								<div class="col-xl-6 col-lg-6 col-md-6 wow fadeInUp" data-wow-delay=".6s">
									<div class="choose-us-box-style-2">
										<div class="icon">
											<i class="lnr-icon-folder-open"></i>
										</div>
										<div class="content">
											<h3><a href="page-service-details.html">Visa Process</a></h3>
											<p>
												End-to-end visa guidance, including documents, SOPs, and interview preparation.
											</p>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<!--How It Work Section -->
	<section class="how-it-work-section fix section-padding">
		<div class="circle-shape">
			<img src="images/home-3/footer-rightshape.png" alt="img">
		</div>
		<div class="auto-container">
			<div class="sec-title text-center">
				<h6 class="sub-title wow fadeInUp">
					<span class="triangle triangle1"></span>
					<span class="triangle triangle2"></span>
					How It Work
				</h6>
				<h2 class="wow fadeInUp" data-wow-delay=".2s">
					Connect, Collaborate, & Create <br> Limitless Opportunities.
				</h2>
			</div>
			<div class="how-it-work-wrapper">
				<div class="row">
					<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 wow fadeInUp" data-wow-delay=".2s">
						<div class="how-it-work-items-style-3">
							<h3 class="number">01</h3>
							<h4 class="title">Free Consultation</h4>
							<p class="text">Talk to our experts and get personalized study abroad guidance.</p>
						</div>
					</div>
					<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 wow fadeInUp" data-wow-delay=".4s">
						<div class="how-it-work-items-style-3">
							<h3 class="number">02</h3>
							<h4 class="title">Course & Country Selection</h4>
							<p class="text">We help you choose the right university and program.</p>
						</div>
					</div>
					<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 wow fadeInUp" data-wow-delay=".6s">
						<div class="how-it-work-items-style-3">
							<h3 class="number">03</h3>
							<h4 class="title">IELTS & Loan Support</h4>
							<p class="text">Prepare for IELTS and get help with education loan approvals.</p>
						</div>
					</div>
					<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 wow fadeInUp" data-wow-delay=".8s">
						<div class="how-it-work-items-style-3 border-none">
							<h3 class="number">04</h3>
							<h4 class="title">Visa & Travel Assistance</h4>
							<p class="text">Complete visa process support and pre-departure guidance.</p>
						</div>
					</div>
				</div>
			</div>	
		</div>
	</section>

	
	<!--Service Section -->
	<section class="growth-section section-padding bg-cover" style="background-image: url('images/home-3/growth-bg.jpg');">
		<div class="sec-shape">
			<img class="sway_Y__animation" src="images/growth-shape.png" alt="Image">
		</div>
		<div class="auto-container">
			<div class="sec-title">
				<div class="row g-4 justify-content-between align-items-center">
					<div class="col-xl-7 col-lg-7">
						<h6 class="text-white sub-title wow fadeInUp">
							<span class="triangle triangle1"></span>
							<span class="triangle triangle2"></span>
							Growth Rate
						</h6>
						<h2 class="text-white wow fadeInUp" data-wow-delay=".2s">Building Connections for grow Limitless Opportunities.</h2>
					</div>
					<div class="col-xl-3 col-lg-4 wow fadeInUp" data-wow-delay=".5s">
						<a href="contact" class="theme-btn btn-style-one wow fadeInUp animated" data-wow-delay=".5s">
							Lets Talk With Us
							<span class="icon"><i class="flaticon-finance-arrow-right-svgrepo-com1-2"></i></span>
						</a>
					</div>
				</div>
			</div>
			<div class="row g-5">
				<div class="col-xl-4 wow fadeInUp" data-wow-delay=".3s">
					<div class="growth-block">
					<!-- pie-graph -->
					<div class="pie-graph">
						<div class="graph-outer">
						<input type="text" class="dial" data-fgColor="#EFCF06" data-bgColor="#fff" data-width="120"
							data-height="120" data-linecap="normal" value="90">
						<div class="inner-text count-box"><span class="count-text" data-stop="90" data-speed="2000"></span>%
						</div>
						</div>
					</div>
					<div class="content-box">
						<h4 class="title">Scalable Solutions</h4>
						<p class="text">End to end fiber optic cable
						nnectivity for stable</p>
					</div>
					</div>
				</div>

				<div class="col-xl-4 wow fadeInUp" data-wow-delay=".5s">
					<div class="growth-block">
					<!-- pie-graph -->
					<div class="pie-graph">
						<div class="graph-outer">
						<input type="text" class="dial" data-fgColor="#EFCF06" data-bgColor="#fff" data-width="120"
							data-height="120" data-linecap="normal" value="95">
						<div class="inner-text count-box"><span class="count-text" data-stop="95" data-speed="2000"></span>%
						</div>
						</div>
					</div>
					<div class="content-box">
						<h4 class="title">Automation Features</h4>
						<p class="text">End to end fiber optic cable
						nnectivity for stable</p>
					</div>
					</div>
				</div>

				<div class="col-xl-4 wow fadeInUp" data-wow-delay=".7s">
					<div class="growth-block">
					<!-- pie-graph -->
					<div class="pie-graph">
						<div class="graph-outer">
						<input type="text" class="dial" data-fgColor="#EFCF06" data-bgColor="#fff" data-width="120"
							data-height="120" data-linecap="normal" value="75">
						<div class="inner-text count-box"><span class="count-text" data-stop="75" data-speed="2000"></span>%
						</div>
						</div>
					</div>
					<div class="content-box">
						<h4 class="title">24/7 Support</h4>
						<p class="text">End to end fiber optic cable
						nnectivity for stable</p>
					</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<!--Team Section -->
	<section class="team-section-3 fix section-padding">
		<div class="auto-container">
			<div class="sec-title text-center">
				<h6 class="sub-title wow fadeInUp">
					<span class="triangle triangle1"></span>
					<span class="triangle triangle2"></span>
					Our Teammate
				</h6>
				<h2 class="wow fadeInUp" data-wow-delay=".3s">
					Enhance Your Experience with <br> Expert Consulting
				</h2>
			</div>
			<div class="row">
				<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 wow fadeInUp" data-wow-delay=".2s">
					<div class="team-box-items-4">
						<div class="team-image">
							<img src="images/home-3/team/team-01.jpg" alt="img">
							<img src="images/home-3/team/team-01.jpg" alt="img">
						</div>
						<div class="socials">
							<i class="fa-solid fa-plus"></i>
							<ul>
								<li><a href="#0"><i class="fa-brands fa-facebook-f"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-instagram"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-linkedin-in"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-behance"></i></a></li>
							</ul>
						</div>
						<div class="content-box">
							<h3 class="title"><a href="page-team-details.html">Guy Hawkins</a></h3>
							<p class="sub-title">Admin</p>
						</div>
					</div>
				</div>
				<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 wow fadeInUp" data-wow-delay=".4s">
					<div class="team-box-items-4">
						<div class="team-image">
							<img src="images/home-3/team/team-02.jpg" alt="img">
							<img src="images/home-3/team/team-02.jpg" alt="img">
						</div>
						<div class="socials">
							<i class="fa-solid fa-plus"></i>
							<ul>
								<li><a href="#0"><i class="fa-brands fa-facebook-f"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-instagram"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-linkedin-in"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-behance"></i></a></li>
							</ul>
						</div>
						<div class="content-box">
							<h3 class="title"><a href="page-team-details.html">Jacob Jones</a></h3>
							<p class="sub-title">Manager</p>
						</div>
					</div>
				</div>
				<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 wow fadeInUp" data-wow-delay=".6s">
					<div class="team-box-items-4">
						<div class="team-image">
							<img src="images/home-3/team/team-03.jpg" alt="img">
							<img src="images/home-3/team/team-03.jpg" alt="img">
						</div>
						<div class="socials">
							<i class="fa-solid fa-plus"></i>
							<ul>
								<li><a href="#0"><i class="fa-brands fa-facebook-f"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-instagram"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-linkedin-in"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-behance"></i></a></li>
							</ul>
						</div>
						<div class="content-box">
							<h3 class="title"><a href="page-team-details.html">Kristin Watson</a></h3>
							<p class="sub-title">Consultant</p>
						</div>
					</div>
				</div>
				<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 wow fadeInUp" data-wow-delay=".8s">
					<div class="team-box-items-4">
						<div class="team-image">
							<img src="images/home-3/team/team-04.jpg" alt="img">
							<img src="images/home-3/team/team-04.jpg" alt="img">
						</div>
						<div class="socials">
							<i class="fa-solid fa-plus"></i>
							<ul>
								<li><a href="#0"><i class="fa-brands fa-facebook-f"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-instagram"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-linkedin-in"></i></a></li>
								<li><a href="#0"><i class="fa-brands fa-behance"></i></a></li>
							</ul>
						</div>
						<div class="content-box">
							<h3 class="title"><a href="page-team-details.html">Bessie Cooper</a></h3>
							<p class="sub-title">Founder</p>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<!--Testimonial Section -->
	<section class="testimonial-section-3 fix section-padding">
		<div class="auto-container">
			<div class="swiper testimonial-slider-3">
				<div class="swiper-wrapper">
					<div class="swiper-slide">
						<div class="testimonial-wrapper-3">
							<div class="mask-shape-style-3">
								<img src="images/home-3/testimonial/mask-shape.png" alt="img">
							</div>
							<div class="image-box-style-3">
								<div class="quate">
									<i class="flaticon-finance-Quote"></i>
								</div>
								<div class="image">
									<img src="images/home-3/testimonial/tesimonial-01.jpg" alt="Image">
									<div class="planet-shape">
										<img src="images/home-3/testimonial/planet.png" alt="img">
									</div>
								</div>
								<div class="star">
									<i class="fa-solid fa-star"></i>
									<i class="fa-solid fa-star"></i>
									<i class="fa-solid fa-star"></i>
									<i class="fa-solid fa-star"></i>
									<i class="fa-solid fa-star"></i>
								</div>
							</div>
							<div class="content-box-style-3">
								<h3 class="text">Ascend the mountain not to plant your flag, but to embrace the challenge,
								savour the
								journey, and marvel at the view.
								Climb to experience the world, not for the world to notice you. This is why they stand
								out with exceptional</h3>
								<h3 class="title">Jacob Jones / <span>Business Ownerr</span></h3>
							</div>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="testimonial-wrapper-3">
							<div class="mask-shape-style-3">
								<img src="images/home-3/testimonial/mask-shape.png" alt="img">
							</div>
							<div class="image-box-style-3">
								<div class="quate">
									<i class="flaticon-finance-Quote"></i>
								</div>
								<div class="image">
									<img src="images/home-3/testimonial/tesimonial-02.jpg" alt="Image">
									<div class="planet-shape">
										<img src="images/home-3/testimonial/planet.png" alt="img">
									</div>
								</div>
								<div class="star">
									<i class="fa-solid fa-star"></i>
									<i class="fa-solid fa-star"></i>
									<i class="fa-solid fa-star"></i>
									<i class="fa-solid fa-star"></i>
									<i class="fa-solid fa-star"></i>
								</div>
							</div>
							<div class="content-box-style-3">
								<h3 class="text">Ascend the mountain not to plant your flag, but to embrace the challenge,
								savour the
								journey, and marvel at the view.
								Climb to experience the world, not for the world to notice you. This is why they stand
								out with exceptional</h3>
								<h3 class="title">Devid Miller / <span>Business Ownerr</span></h3>
							</div>
						</div>
					</div>
				</div>
                <div class="array-button justify-content-end">
                  <button class="array-prev"><i class="fa-regular fa-arrow-left"></i></button>
                  <button class="array-next"><i class="fa-regular fa-arrow-right"></i></button>
                </div>
			</div>
		</div>
	</section>

	<!--Contact Section -->
	<section class="contact-section-3 fix section-padding">
		<div class="auto-container">
			<div class="contact-wrapper-3">
				<div class="row g-4 justify-content-between align-items-center">
					<div class="col-xl-5 col-lg-6">
						<div class="contact-content-style-3">
							<div class="sec-title mb-0">
								<h6 class="sub-title wow fadeInUp">
									<span class="triangle triangle1"></span>
									<span class="triangle triangle2"></span>
									Our Contact
								</h6>
								<h2 class="wow fadeInUp" data-wow-delay=".3s">
									let's work together
								</h2>
							</div>
							<p class="contact-text wow fadeInUp" data-wow-delay=".5s">
								We denounce with righteous indignation and like men
								beguiled and demoralized by the charms
							</p>
							<ul class="contact-list wow fadeInUp" data-wow-delay=".7s">
								<li>
									<div class="icon">
										<i class="lnr-icon-phone-handset"></i>
									</div>
									<div class="content">
										<span>Call For Inquiry</span>
										<h3><a href="tel:+00022200000">+000 (222) 000 00</a></h3>
									</div>
								</li>
								<li>
									<div class="icon">
										<i class="lnr-icon-envelope"></i>
									</div>
									<div class="content">
										<span>Send Us Email</span>
										<h3><a href="/cdn-cgi/l/email-protection#d9b0b7bfb699bca1b8b4a9b5bcf7bab6b4"><span class="__cf_email__" data-cfemail="1b72757d745b7e637a766b777e35787476">[email&#160;protected]</span></a></h3>
									</div>
								</li>
							</ul>
						</div>
					</div>
					<div class="col-xl-6 col-lg-6">
						<div class="contact-box-style-3">
							<h3 class="wow fadeInUp" data-wow-delay=".2s">Need Help For Project!</h3>
							<p class="wow fadeInUp" data-wow-delay=".4s">We are ready to help your next projects, let’s work together</p>
							<form action="#0">
								<div class="row g-4">
									<div class="col-sm-6 wow fadeInUp">
										<div class="input">
											<div class="icon">
												<i class="lnr-icon-phone-handset"></i>
											</div>
											<input type="text" placeholder="Name">
										</div>
									</div>
									<div class="col-sm-6 wow fadeInUp" data-wow-delay=".2s">
										<div class="input">
											<div class="icon"><i class="lnr-icon-envelope"></i></div>
											<input type="email" placeholder="Email">
										</div>
									</div>
								</div>
								<select class="mt-4 wow fadeInUp" data-wow-delay=".4s" name="#" id="subject">
											<option value="0">Choose Our Service</option>
											<option value="2">UI/UX Design</option>
											<option value="3">Web Developer</option>
											<option value="4">Marketing Manager</option>
											<option value="6">Web Designer</option>
											<option value="7">Financial Advice</option>
									</select>
									<textarea class="mt-4 wow fadeInUp" data-wow-delay=".6s" name="masage" id="masage" placeholder="Write a Message"></textarea>
									<button type="submit" class="mt-20 theme-btn btn-style-one wow fadeInUp" data-wow-delay=".8s">
										Send Message
										<span class="icon"><i class="flaticon-finance-arrow-right-svgrepo-com1-2"></i></span>
									</button>
							</form>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="contact-image">
			<img class="wow bounceInRight" data-wow-delay="200ms" data-wow-duration="1500ms"
			src="images/home-3/contact.png" alt="Image">
		</div>
	</section>

	<!--Faq Section -->
	<section class="faq-section-2 fix">
		<div class="faq-wrapper-2">
			<div class="dotted-shape">
				<img src="images/home-3/dotted-map.png" alt="img">
			</div>
			<div class="row g-4 align-items-center">
				<div class="col-xxl-4">
					<div class="faq-image-style-3 image-pllx fix">
						<img src="images/home-3/faq-image.jpg" alt="img">
					</div>
				</div>
				<div class="col-xxl-8">
					<div class="faq-items-wrap">
						<div class="row g-5">
							<div class="col-xl-5 col-lg-6">
								<div class="faq-content-style-3">
									<div class="sec-title mb-0">
										<h6 class="text-white sub-title wow fadeInUp">
											<span class="triangle triangle1"></span>
											<span class="triangle triangle2"></span>
											FAQ
										</h6>
										<h2 class="text-white wow fadeInUp" data-wow-delay=".2s">
											Top Questions Answered
										</h2>
									</div>
									<p class="faq-text wow fadeInUp" data-wow-delay=".4s">
										Everyone with high and useful reward for his Financial statement analysis, Cost-benefit analysis everyone with high and useful reward for his
									</p>
									<a href="contact" class="theme-btn btn-style-one wow fadeInUp animated" data-wow-delay=".6s">
									Lets Talk With Us
									<span class="icon"><i class="flaticon-finance-arrow-right-svgrepo-com1-2"></i></span>
								</a>
								</div>
							</div>
							<div class="col-xl-7 col-lg-6">
								<div class="faq-accordion-style-3">
									<div class="accordion" id="accordionExample2">
										<div class="accordion-item wow fadeInUp">
											<h2 class="accordion-header">
												<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
													data-bs-target="#collapseFive2" aria-expanded="false" aria-controls="collapseFive2">
												What services do you offer?
												</button>
											</h2>
											<div id="collapseFive2" class="accordion-collapse collapse" data-bs-parent="#accordionExample2">
												<div class="accordion-body">
													<p class="text">We offer IELTS coaching, admission guidance, education loan assistance, visa support, and complete study abroad services.
													</p>
												</div>
											</div>
										</div>
										<div class="accordion-item wow fadeInUp" data-wow-delay=".2s">
											<h2 class="accordion-header">
												<button class="accordion-button" type="button" data-bs-toggle="collapse"
													data-bs-target="#collapseOne2" aria-expanded="true" aria-controls="collapseOne2">
												Do you provide assistance for all countries?
												</button>
											</h2>
											<div id="collapseOne2" class="accordion-collapse collapse show"
												data-bs-parent="#accordionExample2">
												<div class="accordion-body">
													<p class="text">Yes, we help students apply to top destinations like the UK, USA, Canada, Dubai, New Zealand, and Europe.
													</p>
												</div>
											</div>
										</div>
										<div class="accordion-item wow fadeInUp" data-wow-delay=".4s">
											<h2 class="accordion-header">
												<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
													data-bs-target="#collapseTwo2" aria-expanded="false" aria-controls="collapseTwo2">
												Can I get a loan without collateral?
												</button>
											</h2>
											<div id="collapseTwo2" class="accordion-collapse collapse" data-bs-parent="#accordionExample2">
												<div class="accordion-body">
													<p class="text">Yes, we assist with both secured and unsecured education loans through our partner banks and NBFCs.
													</p>
												</div>
											</div>
										</div>
										<div class="accordion-item wow fadeInUp" data-wow-delay=".6s">
											<h2 class="accordion-header">
												<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
													data-bs-target="#collapseThree2" aria-expanded="false" aria-controls="collapseThree2">
												Is IELTS mandatory for studying abroad?
												</button>
											</h2>
											<div id="collapseThree2" class="accordion-collapse collapse" data-bs-parent="#accordionExample2">
												<div class="accordion-body">
													<p class="text">It depends on the country and university. Some accept MOI (Medium of Instruction) or other tests like Duolingo or PTE.
													</p>
												</div>
											</div>
										</div>
										<div class="accordion-item wow fadeInUp" data-wow-delay=".8s">
											<h2 class="accordion-header">
												<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
													data-bs-target="#collapseFour2" aria-expanded="false" aria-controls="collapseFour2">
												How do I start my application process?
												</button>
											</h2>
											<div id="collapseFour2" class="accordion-collapse collapse" data-bs-parent="#accordionExample2">
												<div class="accordion-body">
													<p class="text">Simply book a free consultation with us. Our experts will guide you through every step, from course selection to visa.
													</p>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<!-- News Section Start -->
	<section class="news-secton section-padding fix">
		<div class="auto-container">
			<div class="sec-title text-center">
				<h6 class="sub-title wow fadeInUp">
					<span class="triangle triangle1"></span>
					<span class="triangle triangle2"></span>
					Our Blog
				</h6>
				<h2 class="wow fadeInUp" data-wow-delay=".3s">Insights from Latest Blog </h2>
			</div>
			<div class="row">
				<div class="col-xl-4 col-lg-4 col-md-6 wow fadeInUp" data-wow-delay=".3s">
				<div class="news-box-items">
					<div class="news-image">
					<img src="images/home-1/news/news-01.jpg" alt="img" />
					<img src="images/home-1/news/news-01.jpg" alt="img" />
					<span class="post-box">
						<span class="date">20</span>
						<span class="month">March</span>
					</span>
					</div>
					<div class="news-content">
                  <ul class="comments-list">
                    <li><i class="lnr-icon-user1"></i> Admin</li>
                    <li><i class="lnr-icon-comment"></i> 3 Comments</li>
                  </ul>
					<h3>
						<a href="javascript:void(0)">Errors with financial consequences in tax ?</a>
					</h3>
					<a href="javascript:void(0)" class="link-btn">
						Learn More
						<span class="icon">
						<i class="lnr-icon-arrow-right"></i>
						</span>
					</a>
					</div>
				</div>
				</div>
				<div class="col-xl-4 col-lg-4 col-md-6 wow fadeInUp" data-wow-delay=".5s">
				<div class="news-box-items">
					<div class="news-image">
					<img src="images/home-1/news/news-02.jpg" alt="img" />
					<img src="images/home-1/news/news-02.jpg" alt="img" />
					<span class="post-box">
						<span class="date">25</span>
						<span class="month">March</span>
					</span>
					</div>
					<div class="news-content">
                  <ul class="comments-list">
                    <li><i class="lnr-icon-user1"></i> Admin</li>
                    <li><i class="lnr-icon-comment"></i> 3 Comments</li>
                  </ul>
					<h3>
						<a href="javascript:void(0)">Achieving Work Life Balance in a Hectic World</a>
					</h3>
					<a href="javascript:void(0)" class="link-btn">
						Learn More
						<span class="icon">
						<i class="lnr-icon-arrow-right"></i>
						</span>
					</a>
					</div>
				</div>
				</div>
				<div class="col-xl-4 col-lg-4 col-md-6 wow fadeInUp" data-wow-delay=".7s">
				<div class="news-box-items">
					<div class="news-image">
					<img src="images/home-1/news/news-03.jpg" alt="img" />
					<img src="images/home-1/news/news-03.jpg" alt="img" />
					<span class="post-box">
						<span class="date">30</span>
						<span class="month">March</span>
					</span>
					</div>
					<div class="news-content">
                  <ul class="comments-list">
                    <li><i class="lnr-icon-user1"></i> Admin</li>
                    <li><i class="lnr-icon-comment"></i> 3 Comments</li>
                  </ul>
					<h3>
						<a href="javascript:void(0)">Mastering Work-Life On Target Business Goal </a>
					</h3>
					<a href="javascript:void(0)" class="link-btn">
						Learn More
						<span class="icon">
						<i class="lnr-icon-arrow-right"></i>
						</span>
					</a>
					</div>
				</div>
				</div>
			</div>
		</div>
	</section>


    <?php
include __DIR__ . '/A_Layout/Footer/footer.php';
?>